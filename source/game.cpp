...
void Game::make_nexts(Node *node)
{
    Node parent = *node;

    std::pair<int, int> position = parent.board->empty_position;

    if (position.first != 0)
    {
        Node *child_1 = new Node(parent, parent.board);
        ...
        if (this->check_open_closed(*child_1, key))
        {
    ...
    if (position.first != (GAME_SIZE - 1))
    {
        Node *child_2 = new Node(parent, parent.board);
        ...
        if (this->check_open_closed(*child_2, key))
        {
    ...
    {
        Node *child_3 = new Node(parent, parent.board);
        ...
        if (this->check_open_closed(*child_3, key))
        {
    ...
    if (position.second != (GAME_SIZE - 1))
    {
        Node *child_4 = new Node(parent, parent.board);
        ...
        if (this->check_open_closed(*child_4, key))
        {
...
bool Game::check_open_closed(Node &node, std::string key)
{
